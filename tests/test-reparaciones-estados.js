...
// desde el ingreso, y los equipos abiertos de más de 60 días desaparecían.
...
// ── Firestore falso ──
const W = { updates: [], sets: [], batches: [], listeners: [] };
...
c = card({ id: 'g2', nOrden: 15, estado: 'entregado', fase: 'entregado', fechaIngreso: dAgo(40), fechaEntrega: dAgo(10), diasGarantia: 90 });
ok(/quedan 80 días/.test(c), 'entregado: cuenta desde la ENTREGA (90 − 10 = 80)', c.match(/rep-dias-badge[^<]*/));
c = card({ id: 'g3', nOrden: 16, estado: 'entregado', fase: 'entregado', fechaIngreso: dAgo(200), fechaEntrega: dAgo(100), diasGarantia: 90 });
...
setRepairs([
  { id: 'i1', nOrden: 30, estado: 'listo', fase: 'listo' },
  { id: 'i2', nOrden: 31, estado: 'reparando', fase: 'repuesto' },
  { id: 'i3', nOrden: 32, estado: 'reparando', fase: 'ingresado' },
  { id: 'i4', nOrden: 33, estado: 'reparando', fase: 'presupuestado' },
]);
...
console.log('\n10) Los equipos que siguen en el local no desaparecen a los 60 días');
W.listeners = [];
run('_repairsLoaded = false; listenRepairs()');
const ventana  = W.listeners.find(l => l.filtros.some(f => f[0] === 'fechaIngreso'));
const abiertas = W.listeners.find(l => l.filtros.some(f => f[0] === 'estado'));
ok(!!ventana, 'sigue la ventana de 60 días');
ok(abiertas && abiertas.filtros[0][1] === 'in' && abiertas.filtros[0][2].join() === 'reparando,listo',
   'y aparte, SOLO los abiertos (no la colección entera)', abiertas && abiertas.filtros);
const snap = docs => ({ docs: docs.map(x => ({ id: x.id, data: () => x })), docChanges: () => docs, metadata: {} });
ventana.cb(snap([{ id: 'j1', nOrden: 40, estado: 'reparando', fechaIngreso: dAgo(3) }]));
abiertas.cb(snap([{ id: 'j1', nOrden: 40, estado: 'reparando', fechaIngreso: dAgo(3) },
                  { id: 'j2', nOrden: 41, estado: 'listo', fase: 'abandonado', fechaIngreso: dAgo(95) }]));
const ids = get('REPAIRS').map(r => r.id).sort();
ok(ids.join() === 'j1,j2', 'el abandonado de hace 95 días está, y el repetido va una sola vez', ids);

console.log('\n11) Filtros sin opciones repetidas');
